    ...
    for type in types:
        ...
        for filter in filters:
            ...
            for amount in filter_amount:
                ...
                np.savetxt("data/"+file+"/"+type+"/"+filter_name+"/"+type+"-"+filter_name+"-"+name+".csv", filtred, delimiter=",")
                
                A, B, k = GreConD(filtred)
                np.savetxt("data/"+file+"/"+type+"/"+filter_name+"/GreConD/"+type+"-"+filter_name+"-"+name+"-grecond-A.csv", A, delimiter=",")
                np.savetxt("data/"+file+"/"+type+"/"+filter_name+"/GreConD/"+type+"-"+filter_name+"-"+name+"-grecond-B.csv", B, delimiter=",")
                np.savetxt("data/"+file+"/"+type+"/"+filter_name+"/GreConD/"+type+"-"+filter_name+"-"+name+"-grecond-k.txt", np.array([k]), fmt="%d")

                for factor in factors:                    
                    C, D = asso2(filtred, factor, 0.9, 1, 1)
                    np.savetxt("data/"+file+"/"+type+"/"+filter_name+"/ASSO/"+type+"-"+filter_name+"-"+name+"-asso-"+str(factor)+"-A.csv", C, delimiter=",") 
                    np.savetxt("data/"+file+"/"+type+"/"+filter_name+"/ASSO/"+type+"-"+filter_name+"-"+name+"-asss-"+str(factor)+"-B.csv", D, delimiter=",") 

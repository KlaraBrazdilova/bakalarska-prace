...
#barycenter-bfp-deleted-band-paleo
files = ["healthcare", "zoo", "paleo", "mushroom"] #"mushroom", "paleo"
types = ["barycenter-bfp", "spectral-ordering-pearson-bfp", "alternating", "barycenter", "barycenter-bfp-alternating"]
filters = [("deleted-band", ["30", "50", "70", "90"]), 
           ("square-filter",["0.2", "0.3", "0.4", "0.5", "0.35"]),
...
for file in files:
    for type in types:
        for factor in factors:
            ...
            np.savetxt("data/"+file+"/"+type+"/"+type+"-asso-"+str(factor)+"-A.csv", A, delimiter=",") 
            np.savetxt("data/"+file+"/"+type+"/"+type+"-asso-"+str(factor)+"-B.csv", B, delimiter=",") 

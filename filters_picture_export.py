    ...
    for type in types:
        for filter in filters:
            ...
            for amount in filter_amount:
                ...
                axs.imshow(filtered, cmap=newcmp_black_white, aspect='auto', interpolation='nearest') #pro mushroom aspect='auto', interpolation='nearest'
                axs.set_title(folder + "-" + type + "-" + filter_name + "-" + amount)
                plt.savefig("data/"+folder+"/"+type+"/"+filter_name+"/"+type+"-"+filter_name+"-"+amount+".png")       
                matplotlib.pyplot.close()

                fig, axs = plt.subplots(1, 1, figsize=(12, 9)) 
                axs.imshow(changes, cmap=newcmp, aspect='auto', interpolation='nearest') # pro mushroom aspect='auto', interpolation='nearest'
